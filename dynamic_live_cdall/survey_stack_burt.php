    ...
    <?php     
     ...
     { // Only Fire Pixel on the first page offer in the path.
	   ...
	   //   }
		...
		if($_SESSION['user']['revenue_tracker_id']==8203) // Show 8189 Pixel
	     {
 			
			$pixelUrl="https://global.clicktrackurl.com/pixel.do?o=52&t=pb&request_id=".$_SESSION['user']['d2'];
			echo '<iframe src="'.$pixelUrl.'" height="1" width="1" frameborder="0"></iframe>';

	     }

	    if($_SESSION['user']['revenue_tracker_id']==8204) // Show 8189 Pixel
	     {
 			
			$pixelUrl="https://global.clicktrackurl.com/pixel.do?o=51&t=pb&request_id=".$_SESSION['user']['d2'];
			echo '<iframe src="'.$pixelUrl.'" height="1" width="1" frameborder="0"></iframe>';

	     }

		if($_SESSION['user']['revenue_tracker_id']==7841) // Show 8189 Pixel
	     {
 			
			$pixelUrl="http://www.healthylivingfreebies.com/pixel.aspx?tmgsr=".$_SESSION['user']['d2']."&tmgrv=".$_SESSION['user']['d3'];
			echo '<iframe src="'.$pixelUrl.'" height="1" width="1" frameborder="0"></iframe>';

	     }

	    if($_SESSION['user']['revenue_tracker_id']==8043) // Show 8189 Pixel
	     {
 			
			$pixelUrl="http://www.usopinionpoll.com/pixel.aspx?tmgsr=".$_SESSION['user']['d2']."&tmgrv=".$_SESSION['user']['d3'];;
			echo '<iframe src="'.$pixelUrl.'" height="1" width="1" frameborder="0"></iframe>';
